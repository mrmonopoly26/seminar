...
The further section is divided into two subcategories:
\begin{itemize}
    \item Low-level optimizations for \ac{GPU} usage
    \item \ac{SNN} frameworks with \ac{GPU} support.
\end{itemize}

\subsection{Low-level optimizations for \ac{GPU} usage}
\textit{Low-level optimizations} refers to optimizations that exploit either the architecture of the \ac{GPU} or the structure of the \ac{SNN}, which leads to performance improvements or tackling hurdles.
There are several examples in the literature, e.g. ~\cite{nageswaran_efficient_2009},~\cite{knight_efficient_2022}. 
This section focuses on the \textit{Temporal Fusion} method proposed by Li et al., a recent approach for optimizing the performance of \ac{LIF} neurons on single and multi-\acp{GPU}.
...
A key observation is that each neuron depends only on its own state and inputs: its own previous output \( y_i^{(t-1)} \), current input \( x_i^{(t)} \), and membrane potential \( v_i^{(t-1)} \).
...
This method can be applied in both single- and multi-\ac{GPU} environments~\cite{li_towards_2024}.

Fig.~\ref{fig:temporalfusion} schematically illustrates the Temporal Fusion approach.
% TODO paint overhead
...
This section presents and explains frameworks for running and training \acp{SNN} on \acp{GPU}.
...
In recent years, however, a growing demand for machine learning applications has led to the development of frameworks focusing on data science-related tasks~\cite{garcia_vico}.
In the following, a framework of each category is examined.



% Qu: various simulation environments
